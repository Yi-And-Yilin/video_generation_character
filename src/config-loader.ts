...
/**
 * Backward compatibility aliases: map old agent names to new names.
 * Used by config-loader for agent config merging and by fragment-injector
 * for resolving fragment keys.
 */
export const AGENT_ALIASES: Record<string, string> = {
  brainstormer: "brainstormer-ori",
  commander: "commander-ori",
};
...
/**
 * Load plugin config from ~/.config/opencode/
 * Returns null if file doesn't exist or is invalid
 * @param configDir - Optional override for config directory (for testing)
 */
export async function loadPluginConfig(configDir?: string): Promise<PluginConfig | null> {
  const baseDir = configDir ?? join(homedir(), ".config", "opencode");

  try {
    const content = await readConfigFileAsync(baseDir, "plugin");
    if (!content) return null;

    const raw = parseConfigJson(content);
    return buildPluginConfig(raw);
  } catch {
    return null;
  }
}

function buildPluginConfig(raw: unknown): PluginConfig {
  const parsed = v.safeParse(RawPluginConfigSchema, raw);
  ...
  if (config.agents) {
    pluginConfig.agents = sanitizeAgentsRecord(config.agents);
  }

  if (config.features && typeof config.features === "object") {
    pluginConfig.features = sanitizeFeatures(config.features as Record<string, unknown>);
  }

  const threshold = sanitizeCompactionThreshold(config.compactionThreshold);
  if (threshold !== undefined) {
    pluginConfig.compactionThreshold = threshold;
  }

  if (config.fragments) {
    pluginConfig.fragments = sanitizeFragments(config.fragments);
  }
 ...
 * 1. Per-agent override in plugin.json (highest)
...
export function mergeAgentConfigs(
  ...
  for (const [name, agentConfig] of Object.entries(pluginAgents)) {
    // Check if user used an old name that has an alias
    const userOverride =
      userConfig?.agents?.[name] ?? (AGENT_ALIASES[name] ? userConfig?.agents?.[AGENT_ALIASES[name]] : undefined);
    merged[name] = mergeOneAgent(agentConfig, userOverride, name, opencodeDefaultModel, isValidModel);
  }
 ...
 */
export function validateAgentModels(userConfig: PluginConfig, providers: ProviderInfo[]): PluginConfig {
