...
// Valibot schemas for plugin.json and opencode.json config validation
...
const PluginFeaturesSchema = v.object({
  mindmodelInjection: v.optional(v.boolean()),
});

/**
 * Schema for the raw plugin.json config file.
 ...
 */
export const RawPluginConfigSchema = v.object({
...
/**
 * Validate features from plugin.json
 */
export function sanitizeFeatures(raw: Record<string, unknown>): v.InferOutput<typeof PluginFeaturesSchema> {
  const result = v.safeParse(PluginFeaturesSchema, raw);
